...
const DEFAULT_MPV_PATH$1 = "/usr/bin/mpv";
const RESTART_DELAY_MS = 1e3;
const SOCKET_CONNECT_RETRY_MS = 200;
const SOCKET_CONNECT_MAX_RETRIES = 15;
class PlayerManager {
  constructor(window) {
    __publicField(this, "mpvProcess", null);
    __publicField(this, "socket", null);
    __publicField(this, "mainWindow");
    __publicField(this, "windowHandle", null);
    __publicField(this, "mpvPath", DEFAULT_MPV_PATH$1);
    __publicField(this, "requestId", 0);
    __publicField(this, "pendingRequests", /* @__PURE__ */ new Map());
    __publicField(this, "socketBuffer", "");
   ...
   */
  async initialize(windowHandle, mpvPath) {
    this.windowHandle = windowHandle;
    this.mpvPath = mpvPath || DEFAULT_MPV_PATH$1;
    await this.spawnMpv();
  ...
  async play(url) {
    ...
    try {
      ...
      const message = error2 instanceof Error ? error2.message : "Failed to play stream";
  ...
  async spawnMpv() {
    ...
    this.mpvProcess.on("error", (error2) => {
      console.error("[PlayerManager] MPV spawn error:", error2.message);
      this.updateState({ status: "error", error: `MPV failed to start: ${error2.message}` });
    });
    this.mpvProcess.on("exit", (code2, signal) => {
      if (this.destroying) return;
      console.warn(`[PlayerManager] MPV exited with code ${code2}, signal ${signal}`);
      this.socket = null;
  ...
  setupSocketListeners() {
    ...
    this.socket.on("error", (error2) => {
      console.error("[PlayerManager] Socket error:", error2.message);
    });
  }
  processSocketBuffer() {
  ...
  handleMpvMessage(message) {
    if ("request_id" in message && message.request_id !== void 0) {
  ...
  handleEndFile(message) {
    var _a;
    const reason = message.reason || ((_a = message.data) == null ? void 0 : _a.reason);
  ...
  sendCommand(command) {
    return new Promise((resolve2, reject) => {
      ...
      const payload = JSON.stringify({ command, request_id: id2 }) + "\n";
      this.pendingRequests.set(id2, { resolve: resolve2, reject });
  ...
  async handleCrashRestart() {
    ...
    try {
      ...
      const message = error2 instanceof Error ? error2.message : "Failed to restart MPV";
  ...
  async killMpvProcess() {
    if (this.mpvProcess) {
      const process2 = this.mpvProcess;
      this.mpvProcess = null;
      try {
        if (this.socket) {
  ...
  */
  ...
  _watch() {
    ...
    if (process$1.platform === "win32" || process$1.platform === "darwin") {
      ...
      __privateSet(this, _watcher, fs__default.watch(directory, { persistent: false, encoding: "utf8" }, (_eventType, filename) => {
...
class NetworkManager {
   ...
   */
  async fetchPlaylist(url, timeoutMs = DEFAULT_TIMEOUT_MS) {
    ...
    try {
      ...
      if (error2 instanceof TypeError) {
        throw new NetworkError(`Network request failed: ${error2.message}`);
      }
      throw new NetworkError(
        `Network request failed: ${error2 instanceof Error ? error2.message : "Unknown error"}`
      );
    } finally {
...
async function validateMpvPath(mpvPath) {
  try {
    await fs$1.promises.access(mpvPath, fs$1.constants.F_OK | fs$1.constants.X_OK);
    return true;
  } catch {
    return false;
  }
...
async function initializePlayer() {
  ...
  try {
    ...
    const message = error2 instanceof Error ? error2.message : "Failed to initialize player";
    console.error("[Main] Player initialization failed:", message);
    playerManager = null;
  }
}
function registerIpcHandlers() {
  ipcMain$1.handle("player:play", async (_event, url) => {
    if (!playerManager) {
      throw new Error("Player not initialized. Please configure a valid MPV path in settings.");
    }
    await playerManager.play(url);
  });
  ipcMain$1.handle("player:stop", async () => {
    if (playerManager) {
      await playerManager.stop();
    }
  });
  ipcMain$1.handle("player:pause", async () => {
    if (playerManager) {
      await playerManager.pause();
    }
  });
  ipcMain$1.handle("player:resume", async () => {
    if (playerManager) {
      await playerManager.resume();
    }
  });
  ipcMain$1.handle("player:set-volume", async (_event, level) => {
    if (playerManager) {
      await playerManager.setVolume(level);
    }
  });
  ipcMain$1.handle("player:set-mute", async (_event, muted) => {
    if (playerManager) {
      await playerManager.setMute(muted);
    }
  });
  ipcMain$1.handle("playlist:fetch", async (_event, url) => {
    return await networkManager.fetchPlaylist(url);
  });
  ipcMain$1.handle("store:get", async (_event, key) => {
    return storeManager.get(key);
  });
  ipcMain$1.handle("store:set", async (_event, key, value) => {
    storeManager.set(
      key,
      value
    );
  });
  ipcMain$1.handle("store:get-all", async () => {
    return storeManager.getAll();
  });
  ipcMain$1.handle("settings:validate-mpv-path", async (_event, mpvPath) => {
    return await validateMpvPath(mpvPath);
  });
  ipcMain$1.on("log:write", (_event, level, tag, message, data) => {
    const prefix = `[Renderer:${tag}]`;
    const args = [prefix, message];
    if (data !== void 0) args.push(data);
    switch (level) {
      case "error":
        console.error(...args);
        break;
